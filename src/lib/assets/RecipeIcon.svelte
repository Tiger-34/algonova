...
// @ts-nocheck
    ...
    import { currently_showing, isOverlayOpen, recipes, showing_icons } from "./shared.svelte";
    ...
    let self_inx = $derived.by(() => {
        return recipes.info.findIndex((el) => el.id == id)
    });

    let inner_ingredients = $derived.by(() => {
        if (recipes.info[self_inx].ingredients.length > 3) {
...
<div id="parent" transition:fade>
    ...
    <div id="clickable"  onclick={() => {showing_icons.value = false; currently_showing.value = recipes.info[self_inx];}}></div>
